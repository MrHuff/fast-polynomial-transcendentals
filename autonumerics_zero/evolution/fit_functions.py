...
def analyze_fit(name, func, domain, degrees, intervals, weight_func=None, sampling='uniform'):
    print(f"\n--- Analyzing {name} (N={intervals}, Deg={degrees}, sampling={sampling}) ---")
    fitter = PolynomialFitter(
        func, domain=domain, degrees=degrees, num_intervals=intervals,
        weight_func=weight_func, sampling=sampling
    )
    fitter.fit()
    ...
    analyzer = PolynomialAnalyzer(fitter, f"{clean_name}_N{intervals}_D{degrees}{suffix}")
...
if __name__ == "__main__":
    ...
    for metric_name, func, domain, strategies in [
        ("Sigmoid_grad_sym", sigmoid_grad, (0.0, 6.0), [
            ("uniform", {"sampling": "uniform", "weight_func": None}),
            ("chebyshev", {"sampling": "chebyshev", "weight_func": None}),
            ("weighted_opt", {"sampling": "chebyshev", "weight_func": lambda x: peak_weight(x, sigma=1.5, scale=10.0)})
        ]),
        ("Tanh_grad_sym", tanh_grad, (0.0, 3.0), [
            ("uniform", {"sampling": "uniform", "weight_func": None}),
            ("chebyshev", {"sampling": "chebyshev", "weight_func": None}),
            ("weighted_opt", {"sampling": "chebyshev", "weight_func": lambda x: peak_weight(x, sigma=1.2, scale=10.0)})
        ]),
        ("Swish_grad_pos", swish_grad, (0.0, 6.0), [
            ("uniform", {"sampling": "uniform", "weight_func": None}),
            ("chebyshev", {"sampling": "chebyshev", "weight_func": None}),
            ("weighted_opt", {"sampling": "chebyshev", "weight_func": lambda x: peak_weight(x, sigma=3.0, scale=10.0)})
        ])
    ]:
        for deg in [3, 4, 5]:
    ...
    for d_bound in [4.0, 5.0, 6.0, 7.0, 8.0]:
        improved_configs.append((f"Sigmoid_grad_sym_D4_Grid_{d_bound}", sigmoid_grad, (0.0, d_bound), 1, 4,
                       {"sampling": "chebyshev", "weight_func": lambda x: peak_weight(x, sigma=1.5, scale=10.0)}))

    # Tanh bounds
    for d_bound in [2.5, 3.0, 3.5, 4.0]:
        improved_configs.append((f"Tanh_grad_sym_D4_Grid_{d_bound}", tanh_grad, (0.0, d_bound), 1, 4,
                       {"sampling": "chebyshev", "weight_func": lambda x: peak_weight(x, sigma=1.2, scale=10.0)}))
    ...
    deep_dive_configs = [
        ...
        ("Sigmoid_grad_sym_D3_Minimax", sigmoid_grad, (0.0, 6.0), 1, 3, {"fix_boundaries": True}),
        ("Tanh_grad_sym_D3_Minimax",    tanh_grad,    (0.0, 3.0), 1, 3, {"fix_boundaries": True}),
        ("Swish_grad_pos_D3_Minimax",   swish_grad,   (0.0, 6.0), 1, 3, {"fix_boundaries": True}),

        ("Sigmoid_grad_sym_D4_Minimax", sigmoid_grad, (0.0, 6.0), 1, 4, {"fix_boundaries": True}),
        ("Tanh_grad_sym_D4_Minimax",    tanh_grad,    (0.0, 3.0), 1, 4, {"fix_boundaries": True}),
        ("Swish_grad_pos_D4_Minimax",   swish_grad,   (0.0, 6.0), 1, 4, {"fix_boundaries": True}),

        ("Sigmoid_grad_sym_D5_Minimax", sigmoid_grad, (0.0, 6.0), 1, 5, {"fix_boundaries": True}),
        ("Tanh_grad_sym_D5_Minimax",    tanh_grad,    (0.0, 3.0), 1, 5, {"fix_boundaries": True}),
        ("Swish_grad_pos_D5_Minimax",   swish_grad,   (0.0, 6.0), 1, 5, {"fix_boundaries": True}),

        ("Sigmoid_grad_sym_D3_Minimax_extended_range", sigmoid_grad, (0.0, 7.0), 1, 3, {"fix_boundaries": True}),
        ("Tanh_grad_sym_D3_Minimax_extended_range",    tanh_grad,    (0.0, 4.0), 1, 3, {"fix_boundaries": True}),
        ("Swish_grad_pos_D3_Minimax_extended_range",   swish_grad,   (0.0, 7.0), 1, 3, {"fix_boundaries": True}),

        ("Sigmoid_grad_sym_D4_Minimax_extended_range", sigmoid_grad, (0.0, 7.0), 1, 4, {"fix_boundaries": True}),
        ("Tanh_grad_sym_D4_Minimax_extended_range",    tanh_grad,    (0.0, 4.0), 1, 4, {"fix_boundaries": True}),
        ("Swish_grad_pos_D4_Minimax_extended_range",   swish_grad,   (0.0, 7.0), 1, 4, {"fix_boundaries": True}),

        ("Sigmoid_grad_sym_D5_Minimax_extended_range", sigmoid_grad, (0.0, 7.0), 1, 5, {"fix_boundaries": True}),
        ("Tanh_grad_sym_D5_Minimax_extended_range",    tanh_grad,    (0.0, 4.0), 1, 5, {"fix_boundaries": True}),
        ("Swish_grad_pos_D5_Minimax_extended_range",   swish_grad,   (0.0, 7.5), 1, 5, {"fix_boundaries": True}),

        # Ultra-extended range fits - wider domains for better boundary behavior
        ("Sigmoid_grad_sym_D5_Minimax_ultra_extended", sigmoid_grad, (0.0, 10.0), 1, 5, {"fix_boundaries": True}),
        ("Tanh_grad_sym_D5_Minimax_ultra_extended",    tanh_grad,    (0.0, 5.0), 1, 5, {"fix_boundaries": True}),
        ("Swish_grad_pos_D5_Minimax_ultra_extended",   swish_grad,   (0.0, 10.0), 1, 5, {"fix_boundaries": True}),
    ]
    ...
    for name, func, domain, intervals, degree, kwargs in deep_dive_configs:
        ...
        fitter = MinimaxPolynomialFitter(func, domain=domain, degrees=degree, num_intervals=intervals, **kwargs)
        fitter.fit()
    ...
    # Sigmoid Forward
    for deg in [2, 3, 4]:
        forward_pass_configs.append((f"Sigmoid_fwd_sym_D{deg}_Minimax", sigmoid, (0.0, 6.0), 1, deg, {"fix_boundaries": True}))

    # Tanh Forward
    for deg in [2, 3, 4]:
        forward_pass_configs.append((f"Tanh_fwd_sym_D{deg}_Minimax", tanh, (0.0, 3.0), 1, deg, {"fix_boundaries": True}))

    # Swish Forward
    # Swish is x * sigmoid(x). [0, 6]
    for deg in [2, 3, 4]:
        forward_pass_configs.append((f"Swish_fwd_pos_D{deg}_Minimax", swish, (0.0, 6.0), 1, deg, {"fix_boundaries": True}))
