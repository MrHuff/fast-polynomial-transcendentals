...
class MinimaxPolynomialFitter:
    ...
    """
    def __init__(self, func, domain=(0.0, 1.0), degrees=None, num_intervals=1,
                 fix_boundaries=True, method='nelder-mead'):
        """
        Args:
        ...
        for i in range(self.num_intervals):
            ...
            if self.fix_boundaries:
                ...
                if deg < 1:
                    ...
                    lob_nodes = 0.5 * (t_min + t_max) + 0.5 * (t_max - t_min) * lob_nodes
            ...
            if len(free_indices) > 0:
                ...
                if self.method == 'simulated_annealing':
                    ...
                    res = brute(objective_sorted_brute, bounds, Ns=20, full_output=True, finish=None)
                    best_internal = np.sort(res[0])

                else: # Default Nelder-Mead
                    res = minimize(objective_wrapper, initial_params, method='Nelder-Mead', tol=1e-5, options={'maxiter': 5000})
                    best_internal = res.x
