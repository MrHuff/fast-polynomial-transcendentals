...
|---|---|---|---|
| Isolated FP16 function speed | `autonumerics_zero/spline_ops/spline_kernels.cu` and `autonumerics_zero/experiments/benchmark_report_function_speed.py` | Explicit D3--D5 forward/backward functions at 4M-element L2 and 268M-element HBM endpoints, with nine alternating rounds | The retained September 2026 GB200 artifact includes every per-round time, driver and extension-binary hashes, and records that its source tree was dirty; a public rerun creates a new measurement |
| B1 dense SiLU | `autonumerics_zero/spline_ops/spline_kernels_bf16.cu` and generated coefficient headers | `python scripts/benchmark_components.py --cases b1` | The retained component row is a unary direct-derivative probe; deployed B1 recomputes the D3 sigmoid polynomial and forms the SiLU derivative algebraically inside packed SwiGLU. Complete-model measurements exercise the deployed path. The historical component artifact also records a dirty source tree whose full delta is unavailable |
...
| Quantitative table audit | `extras/paper/check_paper_tables.py` checks a SHA-bound `main.tex` claim map against released evidence while ignoring identifier columns | All six groups pass numerically at manuscript rounding, including the isolated FP16 speed table. The default result remains `review-required` until Table 2 semantics are resolved; see `extras/paper/FUNCTION_TABLE_REVIEW.md` |
