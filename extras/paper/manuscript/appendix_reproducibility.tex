...
\begin{description}[leftmargin=1.55in,style=nextline]
    ...
    \item[Isolated function timing]
    \path{isolated_function_speedups_fp16.csv} contains the FP16 endpoint
    ratios in \cref{tab:function-speed}, and
    \path{isolated_function_speedups_fp16.provenance.json} records the
    benchmark protocol and manuscript lineage. Raw alternating-round timings
    and environment metadata are in
    \path{isolated_function_speedups_fp16_gb200.json}. The runnable driver is
    \path{autonumerics_zero/experiments/benchmark_report_function_speed.py}.
