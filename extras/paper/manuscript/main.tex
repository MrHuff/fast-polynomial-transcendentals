...
replace native sigmoid, tanh, and sigmoid linear unit (SiLU) with degree-3 or
degree-4 bfloat16 (BF16) or IEEE binary16 (FP16) programs in four GB200 cases:
dense SiLU, tanh-softcapped attention, sigmoid attention, and routed-expert
Swish-gated linear unit (SwiGLU). The programs combine analytical symmetry,
explicit target-format rounding, and packed arithmetic inside the consuming
kernels. B1, B2, and B4 improve complete training-step throughput by 2.7\%,
2.9\%, and 8.0\%; B3 improves sigmoid-attention forward by 7.4\% and the
complete GPU step by 0.3\%. Same-checkpoint open-weight ablations and one paired
pre-training comparison per case extend the evaluation to model behavior. At
common horizons near 100 billion tokens, the final smoothed training-loss
deltas (polynomial minus native) are $+0.079$, $+0.003$, $-0.107$, and $+0.003$
for B1--B4.
\end{abstract}
...
(SiLU). The native paths studied here evaluate these transcendentals through
SFU-backed instruction sequences. A short polynomial can instead use packed
...
transcendental at the same integration site.

We investigate the four integration cases summarized in \cref{tab:cases}.
...
There are two backward strategies. An algebraic backward reuses the forward
value:
\begin{align}
...
\end{equation}
Algebraic backward reuses the forward result and adds only arithmetic, while
forward error enters the gradient through a product or square and can require
range clipping. A direct fit to $f'$ controls gradient error independently and
can exploit the simpler backward parity. This direct program supplies a
surrogate gradient rather than the analytic derivative of the selected forward
polynomial. We measure both gradient error and training loss. The accepted B3
design uses a direct D3 sigmoid value and direct D4 derivative, while B2 reuses
its tanh output algebraically.
...
forward fit with a separately fitted D4 derivative. The next section measures
whether the accepted numerical programs improve their consuming components.
...
complete-model timing. B1 and B4 isolate their activation kernels; B2 and B3
time complete causal attention.
...
128; B4 evaluates 24,576 rows with hidden size 1280.}
...
\midrule
B1 & One-kernel SiLU activation & D3 SiLU & 1.055x & 1.007x \\
...
Packed D3 SiLU measures 1.055x forward speedup and 1.007x backward speedup on
the 58.7M-value BF16 activation benchmark.

\subsubsection{B2: tanh softcap}

B2 replaces tanh in \cref{eq:softcap} inside FA4. Forward uses packed D4, and
backward uses \cref{eq:softcap-grad} without a second tanh evaluation. Complete
...
D3 forward and a D4 derivative; a runtime guard enforces that fitted length.
Complete-attention forward measures 1.074x and backward measures 1.001x.
...
\end{equation}
Its backward computes gradients for both projected inputs in one pass. The
one-kernel activation benchmark measures 1.003x in forward and 1.005x in
backward.
...
\input{appendix_extended_numerics.tex}
...
\input{appendix_exp2.tex}

\subsection{Paired sine/cosine and fused RoPE}
\label{app:rope-negative}

\input{appendix_rope_fit.tex}
\input{appendix_rope_integration.tex}
...
\begingroup
\scriptsize
\input{appendix_reproducibility.tex}
\endgroup
