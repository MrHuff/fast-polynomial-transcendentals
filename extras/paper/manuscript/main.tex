...
bfloat16 (BF16) programs in four GB200 cases: dense SiLU, tanh-softcapped
attention, sigmoid attention, and routed-expert Swish-gated linear unit
...
1.19--2.19x in L2 and 1.00--1.70x in HBM. B1, B2, and B4 improve complete
training-step throughput by 2.7\%, 2.9\%, and 8.0\%; B3 improves
sigmoid-attention forward by 7.4\% and the complete GPU step by 0.3\%.
Same-checkpoint open-weight ablations and one paired pre-training comparison per
case extend the evaluation to model behavior. At common horizons near 100
billion tokens, the final smoothed training-loss deltas (polynomial minus
native) are $+0.079$, $+0.003$, $-0.107$, and $+0.003$ for B1--B4.
\end{abstract}
